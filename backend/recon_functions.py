...
def read_metadata_esrf(path,print_flag=True):
...
def read_data_esrf(path, proj=None, sino=None, downsample_factor=None, prelog=False,
...
def mask_recon(recon,r=None):
    ...
    #npad = int(np.ceil(numrays * np.sqrt(2)) - numrays) // 2 # to make sure the FFT works fast
...
def get_svmbir_cache_dir():
    ...
    # cache_path = '//global/cfs/cdirs/als/users/tomography_notebooks/svmbir_cache' #global svmbir matrices
    # cache_path =  '/pscratch/sd/h/hasitha/svmbir_cache_moon' #Perlmutter scratch - change to your username
    cache_path =  '/pscratch/sd/h/hasitha/svmbir_cache_moon' #rays: 2560, numangles: 2625
    # cache_path = '/global/cscratch1/sd/hasitha/svmbir_cache' #cori scratch - change to your username
    if not path.exists(cache_path):
        raise FileNotFoundError(
            f'File "{cache_path}" does not exist! '
            "Please uncomment the rlevent SVMBIR cache saving path on get_svmbir_cache_dir "
            "function on ALS_recon_functions.py."
            )
    return cache_path
