...
import recon_functions as als
import recon_helper as helper

MAX_JOB_SECONDS = 600*60 # 1 hour 20 min
...
def create_svmbir_batch_script(settings, *argv):
    ...
    with open(config_script_name, 'w') as f:
        ...
        # script += f"srun -N {N} -n {n} python /global/u1/h/hasitha/SVMBIR/ALS_batch_recon.py" # for conda env hasitha
        if argv:
